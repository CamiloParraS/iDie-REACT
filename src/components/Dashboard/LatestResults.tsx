            ...
            {results.map((result) => {
                ...
                return (
                    <article key={result.id} className="idie-card flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="text-sm font-bold text-blue-900">{result.testType}</p>
                            <p className="text-xs text-slate-600">{formatDateTime(result.date)}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-sm font-semibold">
                            <span>
                                ...
                                {result.value === null
                                    ? 'Pendiente'
                                    : `${result.value} (${result.referenceMin}-${result.referenceMax})`}
                            </span>
                            <span
                                className={[
                                    'idie-badge',
